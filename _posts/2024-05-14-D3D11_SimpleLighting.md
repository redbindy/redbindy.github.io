---
layout: post
title: "Direct3D11 튜토리얼 정리6: 간단한 라이팅 구현(야매 버전...)(작성중)"
category: 컴퓨터공학
...
![결과 이미지](assets/images/2024-05-14-D3D11_SimpleLighting_images/result)

&emsp;요렇게 밝기 다른 느낌을 주는 걸 구현해보겠습니다. 사실 라이팅이라고도 할 수 없는 방법인지라 제대로된 건 그래픽스 공부하시면서 하시길 권장!
...
앞서 말씀드린 것에 더해서 그림에서 보실 수 있는 것처럼 법선 벡터와 빛이 들어오는 각도를 통해서 해당 면이 빛을 얼마나 강하게 반사하는지도 반영할 수 있습니다. 이에 대해서 더 자세하게 설명해드리면, 빛이 법선 벡터랑 나란하게 들어온 경우에는 거의 모든 빛을 그대로 반사하게 된다고 볼 수 있습니다. 반면에, 비스듬히 들어왔다면 빛이 퍼지면서 상대적으로 약한 빛이 반사되게 됩니다.(면이 매끈하고 전부 반사된다는 가정) <br>
...
그 이유에 대해서는 아마 이 그림만 보시면 바로 납득이 되실 것 같습니다. 한 정점에 대해서 면 바깥으로 향하는 정점은 위 그림처럼 세 방향으로 정의될 수 있기 때문입니다. 그리고 실제로 모든 방향에 대해서 정의를 해야 각 면에 맞는 법선 벡터를 설정할 수 있기도 합니다.
